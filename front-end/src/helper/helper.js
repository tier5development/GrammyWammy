...
export function OpenFacebookProfileInTab() {
  try {
    const myNewUrl =
      `https://www.instagram.com/` + localStorage.getItem("insta_username");
    let CreateTab = chrome.tabs.create({
      url: myNewUrl,
      active: true,
    });
...
export function framecaller() {
  try {
    const myNewUrl =
      "https://www.instagram.com/" + console.log("This is a ", myNewUrl);
    return myNewUrl;
  } catch (error) {
    console.log("This is a ", error);
  }
...
export function OpenTier5Partnership() {
  try {
    ...
    const myNewUrl =
      "https://partner.tier5.us/jrpartnership666?utm_source=GrammyWammy";
    let CreateTab = chrome.tabs.create({
      url: myNewUrl,
      active: true,
    });
