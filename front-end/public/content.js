...
const postTweet = (closeWindow, id, message) => {
  ...
  setTimeout(() => {
    ...
    let allMessageDiv = document.getElementsByClassName(
      " DPiy6 Igw0E IwRSH eGOV_ _4EzTm "
    );
...
function readLastMessage(id) {
  ...
  setTimeout(() => {
    let totalIncomingMessages =
      document.getElementsByClassName("Igw0E  Xf6Yq          hLiUi    ybXk5")
        .length - 1;
    console.log(totalIncomingMessages);
    let userLastSentMessage = document
      .getElementsByClassName("Igw0E  Xf6Yq   hLiUi    ybXk5")
      [
    ...
    userNameToBeSent = document.getElementsByClassName(
...
chrome.runtime.onMessage.addListener(async function (
  ...
  if (request.catch === "get-login-info") {
    ...
    if ($(".ctQZg").length) {
      ...
      UserInstagramName = document.getElementsByClassName(
        "_7UhW9   xLCgt      MMzan   _0PwGv             fDxYl "
      )[1].innerHTML;
      UserInstagramImage = document.getElementsByClassName("_47KiJ")[0]
        .children[4].children[1].children[0].src;
    ...
    chrome.runtime.sendMessage({
      type: "storeUserInfoOrQueryThenStore",
      options: parameters,
    });
  }
          ...
          if (allMessageDiv[i]) {
            ...
            if (childDiv.childElementCount === 3) {
              let messageLink =
                ...
                allMessageDiv[i].children[0].getAttribute("href");
              let messageId = messageLink.split("/").pop();
              console.log(messageId);
              port.postMessage({
                options: messageId,
                ConFlag: "StoreMessageLinkInLocalStorage",
              });
            }
          }
        }
...
port.onMessage.addListener(async function (msg) {
  if (msg.ConFlagBack === "DEFAULTMESSAGEBACK") {
    ...
    port.postMessage({
      MessageDetails: setDefaultMessageSaveONEX,
      ConFlag: "STOREANDCLOSE",
    });
  }
});
