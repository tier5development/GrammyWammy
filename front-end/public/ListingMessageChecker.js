...
$(document).ready(function () {
  ...
  chrome.runtime.sendMessage({
    type: "TriggerReaderParam",
    options: "MessageListing",
  });
  chrome.runtime.onMessage.addListener(async function (request, sender) {
    if (request.type === "StartTheMutation") {
      if (request.TriggerLayout === "CreateLayout") {
        ...
        textDiv.innerHTML =
          "GrammyWammy Is Using This Tab Please Don`t Close It";
            ...
            .each(async function () {
              ...
              if (CountChildrenDiv === 3) {
                ...
                if (InsideHtml !== "Typing...") {
                  console.log("I am here", InsideHtml, new Date());
                  let port = chrome.runtime.connect({ name: "ListKnock" });
                  port.postMessage({
                    options: messageId,
                    ConFlag: "StoreUrlInStoreAndRetriveDate",
                  });
                  port.disconnect();
                }
              }
            ...
            await $(this)
              ...
              .each(async function (key, laa) {
                ...
                if (messagelinkId) {
                  ...
                  console.log(messageId, SendMessageId, anchor);
    ...
    if (request.type === "ReplyInstaUser") {
      ...
      //   $(`textarea`).focus();
      //   const blob = new Blob([request.options.ReplyMessage], {
      //     type: "text/plain",
      //   });
      //   let cpData = [new ClipboardItem({ "text/plain": blob })];
      //   await navigator.clipboard.write(cpData).then(function () {
      //     $(`textarea`).focus();
      //     setInterval(() => {
      //       $(`textarea`).focus();
      //     }, 500);
...
async function grabMessageInfoAndSend(MessageId) {
  await setTimeout(async function () {
    ...
    $("#react-root")
      ...
      .find(">div")
      .find(">div")
      .each(async function (countx, valx) {
        if (countx === 0) {
          $(this)
            .find("> div")
            .each(async function (county, valy) {
              let mc = $(valy)
                .find(" > div:nth-child(2)")
                .find(" > div")
                .find("> div")
              ...
              if (
                $(valy).find(" > div").find(" > div:nth-child(1)").length !== 1
              ) {
                // let acount=$(valy).find(' > div').find(' > div:nth-child(1)').find("a").length;
                if (
                  $(valy).find(" > div").find(" > div:nth-child(1)").find("a")
                    .length === 1
                ) {
                  UserLink = $(valy)
                    .find(" > div")
    ...
    chrome.runtime.sendMessage({
      type: "CheckMessageAndResponse",
      options: responseMessageDetails,
    });
  }, 3000);
}
